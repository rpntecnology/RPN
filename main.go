...
func main() {
	...
	r.Handle("/finishTask", jwtMiddleware.Handler(http.HandlerFunc(FinishTaskHandler))).Methods("POST")
	r.Handle("/terminateTask", jwtMiddleware.Handler(http.HandlerFunc(TerminateTaskHandler))).Methods("POST")
